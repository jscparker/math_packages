...
procedure gorilla_tst_demo_1 is

  pragma Assert (Random_Int'Last >= 2**32-1);
  --  Random_Int should be uniform on 0..2**n-1 in order to pass test.
  --  with  n >= 32.

  type Real is digits 15;

  package Maths is new  Ada.Numerics.Generic_Elementary_Functions (Real); use Maths;
  package Normal_p_vals is new Chi_Gaussian_CDF (Real); use Normal_p_vals;
  ...
  No_of_Tests : constant Integer := 2**5;
  ...
  type Unsigned_Stnd is mod 2**32;

  package Stnd_Discrete is new Ada.Numerics.Discrete_Random (Unsigned_Stnd);

  Stream_gnat : Stnd_Discrete.Generator;

  procedure Get_Random_g(X : out Random_Int; S : in Stnd_Discrete.Generator) 
  is
  begin
    X := Random_Int (Stnd_Discrete.Random (S) / 2**16);
  end Get_Random_g;
...
begin 
  ...
  for Test_id in 1 .. No_of_Tests loop
    ...
    new_Line;
    put ("ks p-val average (should -> 1/2 after many ks tests):           "); 
    put (Real'Image(ave_p_val / Real (Test_id)));
    new_Line;
    put ("ks p-val variance (normalized, should -> 1 after many ks tests):"); 
    put (Real'Image(ave_variance / (Real (Test_id)*(0.25/3.0)))); -- should -> 1.0
    new_Line;

  end loop;  -- for Test_id in 1..No_of_Tests: get p_vals and average them.
